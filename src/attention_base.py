...
class NeuronAttentionBase(nn.Module):
    ...
    def init_gqa_properties(self):
        ...
        if self.cp_degree == 1 and self.dp_degree > 1:
            ...
            cte_rank_ordering = get_cp8_tp8_rank_ordering(self.neuron_config.tp_degree, self.cp_degree)
        ...
        o_proj = GroupQueryAttention_O(
            ...
            tensor_model_parallel_group=self.tensor_model_parallel_group,
            rpl_reduce_dtype=self.rpl_reduce_dtype,
            out_proj_kernel_enabled=self.attn_block_tkg_nki_kernel_enabled or self.attn_block_cte_nki_kernel_enabled,
            logical_nc_config=self.neuron_config.logical_nc_config,
            tiling_factor=self.neuron_config.cc_pipeline_tiling_factor if self.neuron_config.tile_cc else 1,
            rank_ordering=cte_rank_ordering,
        )
        ...
        # CP > 1 and DP > 1 and CP != DP, CTE runs in TP + CP and Decode in TP + DP
            ...
            rank_ordering = get_context_parallel_reordered_dp_mapping(self.neuron_config.tp_degree, self.cp_degree, self.dp_degree, self.num_key_value_heads, cte_rank_ordering=cte_rank_ordering)
    ...
    def context_parallel_flash_attention_kernel(self, Q, K_active, V_active, q_len, bsz, strategy):
        ...
        cp_rank = get_cp_rank(self.rank_util.get_rank(), self.tp_degree, self.cp_degree)
    ...
    def perform_prefill(self, Q, K, V, q_len, bsz, attention_mask) -> Tensor:
        ...
        if flash_attn_strategy != FlashAttentionStrategy.NONE and self.cp_degree > 1:
            attn_output = self.context_parallel_flash_attention_kernel(Q, K_active, V_active, q_len, bsz, flash_attn_strategy)
    ...
    def perform_prefill_chunked_attn(self, Q, K, V, q_len, bsz, attention_mask, chunk_size) -> Tensor:
        ...
        if flash_attn_strategy != FlashAttentionStrategy.NONE:
            ...
            if flash_attn_strategy == FlashAttentionStrategy.SHARDED_KERNEL:
                ...
                logger.debug(f"prefill chunked attention")
                _flash_fwd_call(
                    Q,
                    K_active,
                    V_active,
    ...
    def get_flash_attention_strategy(self, q_len, has_attention_mask) -> FlashAttentionStrategy:
        ...
        if self.sliding_window:
            ...
            # Note: flash_fwd does not support learned sinks, so use fallback if sinks are present
            if self.learned_sinks_size is not None and self.learned_sinks_size > 0:
                logger.debug("Learned sinks present - using fallback instead of flash_fwd for sliding window")
                return FlashAttentionStrategy.NONE
            if q_len >= MIN_SLIDING_WINDOW_SEQ_TILE_SIZE:
                return FlashAttentionStrategy.SLIDING_WINDOW_KERNEL
            logger.debug("min sliding window")
            return FlashAttentionStrategy.NONE
    ...
    def attention_context_encode_windowed_attention(self, Q, K, V, q_len, bsz, attention_mask, window_size=None):
    ...
    def standard_causal_attention_forward(
        ...
        **kwargs,
        ...
        if is_data_parallel:
            ...
            dp_rank = get_dp_rank(self.rank_util.get_rank(), self.tp_degree, self.dp_degree)
    ...
    def chunked_attention_with_context_parallel_forward(
        ...
        **kwargs,
        ...
        assert not self.sequence_parallel_enabled, (
            "Disable sequence parallel to use chunked attention forward"
        )
        original_dtype = hidden_states.dtype
        hidden_states = hidden_states.to(self.torch_dtype)
        ...
        is_token_gen = past_key_value is not None
        if not is_token_gen and self.neuron_config.sequence_parallel_enabled:
            hidden_states = gather_from_sequence_parallel_region(
                hidden_states,
                1,
                process_group=get_tensor_model_parallel_group(),
            )
            # TODO also gather residual for attn kernels that use residual as input such as attention_block_cte_nki_kernel
        bsz, q_len, _ = hidden_states.size()
        n_chunks = math.ceil(q_len / self.attention_chunk_size)
        ...
        if not is_token_gen:
            ...
            Ks = []
            Vs = []
            should_compute_cos_sin = cos_cache is None or sin_cache is None
            existing_cos = cos_cache.clone() if cos_cache is not None else None
            existing_sin = sin_cache.clone() if sin_cache is not None else None
            for i in range(n_chunks):
                # split inputs into chunks
                chunk_idx = i

                chunked_attn_output, chunked_cos, chunked_sin, chunked_K, chunked_V = self.compute_attention_per_chunk_with_context_parallel(
                    chunk_idx,
                    hidden_states,
                    rotary_position_ids,
                    attention_mask,
                ...
                if should_compute_cos_sin:
                    cos_caches.append(chunked_cos)
                    sin_caches.append(chunked_sin)
                Ks.append(chunked_K)
                Vs.append(chunked_V)
            ...
            if should_compute_cos_sin:
                cos_cache = torch.cat(cos_caches, dim=1) if len(cos_caches) > 0 and cos_caches[0] is not None else None
                sin_cache = torch.cat(sin_caches, dim=1) if len(sin_caches) > 0 and sin_caches[0] is not None else None
            else:
                cos_cache = existing_cos
                sin_cache = existing_sin
            attn_output = self.get_o_proj()(attn_output, adapter_ids=adapter_ids)
    ...
    def compute_attention_per_chunk_with_context_parallel(
        ...
        """
        # split inputs into chunks
        should_compute_cos_sin = cos_cache is None or cos_cache is None
        chunk_start = chunk_idx * self.attention_chunk_size
        chunk_end = min((chunk_idx + 1) * self.attention_chunk_size, hidden_states.shape[1])
        chunked_hidden_states = torch.ops.aten.slice(
            hidden_states, dim=1, start=chunk_start, end=chunk_end
        )
        ...
        attn_output = attn_output.reshape(
            bsz,
            (chunk_end - chunk_start) // self.cp_degree,
        ...
        if chunked_cos is not None and should_compute_cos_sin:
            cos_sin = torch.stack([chunked_cos, chunked_sin], dim=0)
            cos_sin = gather_from_tensor_model_parallel_region_with_dim(
                cos_sin,
                gather_dim=2,
                process_group=get_context_parallel_attention_cp_group(),
            )

            chunked_cos, chunked_sin = torch.unbind(cos_sin, dim=0)

        return attn_output, chunked_cos, chunked_sin, K, V
    ...
    def windowed_attention_forward(
        ...
        **kwargs,
        ...
        if is_token_gen:
            ...
            attn_output, K, V = self.attention_context_encode_windowed_attention(Q, K, V, q_len, bsz, attention_mask, self.sliding_window)
            K, V = get_last_kv_window(self.sliding_window, full_position_ids, K, V)
