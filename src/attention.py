...
def flash_fwd(
    ...
    for i_q_h in nl.affine_range(q_h_per_k_h):
        ...
        for i0 in nl.sequential_range(n_remat):
            ...
            for j in nl.sequential_range(0, num_large_k_tile):
                ...
                for i1 in nl.affine_range(attn_core_tile_size):
                    ...
                    if (i < n_tile_q) & causal_mask & sliding_mask:
                        ...
                        # s_tile = nl.ndarray((B_P_SIZE, 1), dtype=kernel_dtype)
                        # s_hbm_tile = s[batch_id, head_id * q_h_per_k_h + i_q_h]
                        # s_sbuf_tile = nl.load(
                        #     s_hbm_tile[:, nl.ds(i * B_P_SIZE, B_P_SIZE)], dtype=kernel_dtype
                        # )
                        # s_tile[:, :] = s_sbuf_tile
            ...
            for i_s in nl.affine_range(attn_core_tile_size):
                i = i0 * attn_core_tile_size + i_s
                ...
                if i < n_tile_q:
                    ...
                    # m_i_final = max(m_i, s_i)
                    m_i_final = nl.maximum(m_buffer[i_s, :, :], s_tile)
                    
                    # a_sink = exp(m_i - m_i_final)
                    exp = nisa.activation(
                        nl.exp, m_i_final, bias=m_buffer[i_s, :, :], scale=-1.0
                    )
                    
                    # scale o
                    o_buffer[i_s, :, :] = nl.multiply(o_buffer[i_s, :, :], exp, dtype=acc_type)
                    
                    # compute l_i_final
                    exp1 = nisa.activation(
                        nl.exp, m_i_final, bias=l_buffer[:, i], scale=-1.0  # exp(L_old - m_final)
                    )
                    exp2 = nisa.activation(
                        nl.exp, m_i_final, bias=s_tile, scale=-1.0  # exp(s - m_final) [FIXED]
                    )
                    temp = nl.add(exp1, exp2) # TODO: Rename this
                    l_buffer[:, i] = nl.add(m_i_final, nisa.activation(nl.log, temp[:, 0]))
                    m_buffer[i_s, :, :] = m_i_final
