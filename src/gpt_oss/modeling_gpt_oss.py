...
def convert_hf_format_state_dict_bf16_compute(state_dict: dict, config: InferenceConfig):
    ...
    for layer in range(num_layers):
        ...
        for proj in ["down_proj", "gate_up_proj"]:
            ...
            dequantized_weights = state_dict[f"layers.{layer}.mlp.experts.{proj}"]
            ...
            if proj == "gate_up_proj":
                ...
                state_dict[f"layers.{layer}.feed_forward.moe.expert_mlps.mlp_op.{proj}.weight"] = dequantized_weights.contiguous()
                state_dict[f"layers.{layer}.feed_forward.moe.expert_mlps.mlp_op.{proj}.bias"] = state_dict[f"layers.{layer}.mlp.experts.{proj}_bias"]

            # Delete old MoE expert keys after conversion
            del state_dict[f"layers.{layer}.mlp.experts.{proj}"]
...
def _helper_concat_and_delete_qkv(GptOss_state_dict, layer_num, attr):
    ...
    """
    GptOss_state_dict[f"layers.{layer_num}.self_attn.qkv_proj.Wqkv.{attr}"] = torch.cat(
        [
...
class NeuronGptOssMoE(nn.Module):
    def __init__(self, config: InferenceConfig, rmsnorm: Optional[nn.Module] = None):
        ...
        # Disable TKG module to use flat compiler flow instead of NKI kernel
        # Set init_tkg_module=False to disable moe_fused NKI kernel
        self.moe = initialize_moe_module(config=config,
                                         rmsnorm=rmsnorm,
                                         init_tkg_module=False,  # Disabled to use flat compiler flow
                                         router_bias=True,
                                         experts_bias=True,
...
class NeuronGptOssForCausalLM(NeuronBaseForCausalLM):
    ...
    def convert_hf_to_neuron_state_dict(state_dict: dict, config: InferenceConfig) -> dict:
        ...
        state_dict = NeuronGptOssForCausalLM._convert_hf_format_state_dict(state_dict, config)
    ...
    def _convert_hf_format_state_dict(state_dict: dict, config: InferenceConfig) -> dict:
        ...
        if neuron_config.fused_qkv:
            ...
            for layer in range(num_layers): 
                state_dict[f"layers.{layer}.self_attn.qkv_proj.q_proj.weight"] = state_dict[f"layers.{layer}.self_attn.q_proj.weight"]
                state_dict[f"layers.{layer}.self_attn.qkv_proj.q_proj.bias"] = state_dict[f"layers.{layer}.self_attn.q_proj.bias"]
                state_dict[f"layers.{layer}.self_attn.qkv_proj.k_proj.weight"] = state_dict[f"layers.{layer}.self_attn.k_proj.weight"]
                state_dict[f"layers.{layer}.self_attn.qkv_proj.k_proj.bias"] = state_dict[f"layers.{layer}.self_attn.k_proj.bias"]
                state_dict[f"layers.{layer}.self_attn.qkv_proj.v_proj.weight"] = state_dict[f"layers.{layer}.self_attn.v_proj.weight"]
                state_dict[f"layers.{layer}.self_attn.qkv_proj.v_proj.bias"] = state_dict[f"layers.{layer}.self_attn.v_proj.bias"]
                
                del state_dict[f"layers.{layer}.self_attn.q_proj.weight"]
                del state_dict[f"layers.{layer}.self_attn.q_proj.bias"]
                del state_dict[f"layers.{layer}.self_attn.k_proj.weight"]
                del state_dict[f"layers.{layer}.self_attn.k_proj.bias"]
                del state_dict[f"layers.{layer}.self_attn.v_proj.weight"]
                del state_dict[f"layers.{layer}.self_attn.v_proj.bias"]

        # Attention Output Projection
        for layer in range(num_layers):
            state_dict[f"layers.{layer}.self_attn.o_proj.o_proj.weight"] = state_dict[f"layers.{layer}.self_attn.o_proj.weight"]
            del state_dict[f"layers.{layer}.self_attn.o_proj.weight"]
            state_dict[f"layers.{layer}.self_attn.o_proj.o_proj.bias"] = state_dict[f"layers.{layer}.self_attn.o_proj.bias"]
            del state_dict[f"layers.{layer}.self_attn.o_proj.bias"]
    ...
    def _pad_hf_state_dict(state_dict: dict, config: InferenceConfig) -> None:
        ...
        pad_map = {
            ...
            "layers.*.mlp.experts.down_proj": (0, diff_H, 0, 0, 0, 0),
            # "layers.*.mlp.experts.down_proj_scales": (0, diff_I // (16 * 2), 0, diff_H),
            "layers.*.mlp.experts.gate_up_proj_bias": (0, diff_I * 2),
            "layers.*.mlp.experts.gate_up_proj": (0, 0, 0, diff_H, 0, diff_I * 2),
            # "layers.*.mlp.experts.gate_up_proj": (0, 0, 0, 0, 0, diff_H),
        ...
        for key in state_dict.keys():
            ...
            if pad_map_key in pad_map:
                ...
                state_dict[key] = state_dict[key].contiguous() # Ensure tensor is contiguous
    ...
    def _convert_neuron_format_state_dict(state_dict: dict, config: InferenceConfig) -> dict:
        ...
        for layer in range(num_layers):
            ...
            state_dict[f"layers.{layer}.input_layernorm.weight_unpadded"] = state_dict[f"block.{layer}.attn.norm.scale"][:config.original_hidden_size].clone().contiguous()
            ...
            state_dict[f"layers.{layer}.self_attn.q_proj.weight"] = q.contiguous()
            state_dict[f"layers.{layer}.self_attn.k_proj.weight"] = k.contiguous()
            state_dict[f"layers.{layer}.self_attn.v_proj.weight"] = v.contiguous()
            ...
            state_dict[f"layers.{layer}.self_attn.q_proj.bias"] = qb.contiguous()
            state_dict[f"layers.{layer}.self_attn.k_proj.bias"] = kb.contiguous()
            state_dict[f"layers.{layer}.self_attn.v_proj.bias"] = vb.contiguous()
